      ...
      >
        ...
        <td className="px-4 py-4 w-10">
          {isMulti ? (
            ...
            <span className="text-text-secondary text-xs font-mono">#{first.id}</span>
          )}
        </td>
        ...
        <td className="px-4 py-4">
          {isMulti ? (
            <div className="flex items-center gap-2">
              ...
              <div>
                <div className="font-medium text-text-primary text-sm">{items.length} items</div>
            ...
            <div className="flex items-center gap-2">
              <Package className="w-3.5 h-3.5 text-text-secondary shrink-0" />
              <div>
                <div className="font-medium text-text-primary text-sm">{first.product_name}</div>
                <div className="text-xs text-text-secondary font-mono">{first.product_sku}</div>
              </div>
            </div>
          ) : (
            <span className="text-text-secondary italic text-xs">Unknown</span>
          )}
        </td>
        ...
        <td className="px-4 py-4 text-right">
          <span className={`font-mono font-semibold text-sm ${totalQty > 0 ? 'text-green-600' : 'text-red-500'}`}>
            {totalQty > 0 ? '+' : ''}{totalQty}
            {isMulti && <span className="text-xs font-normal text-text-secondary ml-1">total</span>}
          </span>
        </td>
      ...
      <AnimatePresence>
        {isOpen && isMulti && items.map((tx, idx) => (
          <motion.tr
            ...
            className="border-b border-border/20 bg-blue-50/20 hover:bg-blue-50/40 transition-colors"
          >
            {/* Indent marker */}
            <td className="px-4 py-3">
              <span className="font-mono text-xs text-text-secondary pl-2">#{tx.id}</span>
            </td>
            <td className="px-4 py-3" />
            {/* Product */}
            <td className="px-4 py-3">
              <div className="flex items-center gap-2 pl-4 border-l-2 border-blue-200 ml-2">
                <div>
                  <div className="text-sm font-medium text-text-primary">{tx.product_name || '—'}</div>
                  <div className="text-xs text-text-secondary font-mono">{tx.product_sku}</div>
                </div>
              </div>
            </td>
            {/* Batch */}
            ...
            <td className="px-4 py-3 text-right">
              <span className={`font-mono font-semibold text-sm ${tx.quantity > 0 ? 'text-green-600' : 'text-red-500'}`}>
                {tx.quantity > 0 ? '+' : ''}{tx.quantity}
              </span>
            </td>
            <td className="px-4 py-3">
              {tx.notes ? (
                <span className="text-xs text-text-secondary italic">{tx.notes}</span>
              ) : '—'}
            </td>
            <td colSpan="2" />
          </motion.tr>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-start">
        <div>
          <h1 className="text-2xl font-bold text-text-primary tracking-tight">Stock Ledger</h1>
          <p className="text-text-secondary text-sm mt-1">Read-only historical stream of all inventory movements.</p>
        </div>
        <div className="text-sm text-text-secondary bg-surface border border-border rounded-lg px-4 py-2">
      ...
      <Card className="border-border shadow-sm bg-surface/90 backdrop-blur-md">
        <CardHeader className="py-4 border-b border-border/50 bg-slate-50/50">
        ...
        <CardContent className="p-0 overflow-x-auto">
          <table className="w-full text-sm text-left">
            ...
            <tbody>
              {isLoading && (
                <tr>
                  <td colSpan="9" className="px-4 py-10 text-center text-text-secondary">Loading ledger…</td>
                </tr>
              )}
