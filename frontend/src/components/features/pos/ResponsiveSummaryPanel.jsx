      ...
      <div className="flex items-center justify-between p-4 bg-surface border-t border-border shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
        <div className="flex gap-8">
          <div className="flex flex-col">
            ...
            <span className="font-mono text-lg font-medium">Rs {subtotal.toFixed(2)}</span>
          </div>
          <div className="flex flex-col">
            <span className="text-xs text-text-secondary uppercase tracking-wider">Tax</span>
            <span className="font-mono text-lg font-medium">Rs {tax.toFixed(2)}</span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-6">
          ...
          {paymentMethod === 'cash' && Number(amountPaid) > total && (
            <div className="flex flex-col text-right">
              ...
              <span className="font-mono text-xl font-bold text-green-600">Rs {(Number(amountPaid) - total).toFixed(2)}</span>
            </div>
          )}
          ...
          <div className="flex flex-col text-right">
            ...
            <span className="font-mono text-2xl font-bold text-text-primary">Rs {total.toFixed(2)}</span>
          </div>
    ...
    <div className="flex flex-col w-96 h-full bg-surface border-l border-border p-6 shadow-sm shrink-0">
      ...
      <div className="space-y-4 flex-1">
        ...
        <div className="pt-4 border-t border-border mt-4 flex justify-between items-center">
          ...
          <span className="font-mono text-3xl font-bold text-text-primary">Rs {total.toFixed(2)}</span>
        </div>
      </div>
          ...
          <div className="mt-4 p-4 bg-slate-50 border border-border rounded-lg space-y-3">
            ...
            {Number(amountPaid) > total && (
              <div className="flex justify-between items-center pt-2 border-t border-border">
                ...
                <span className="font-mono text-lg font-bold text-green-600">Rs {(Number(amountPaid) - total).toFixed(2)}</span>
              </div>
            )}
          </div>
        )}
